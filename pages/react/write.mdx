...
do not immediately triggers upon render, instead you invoke mutations in
...
in Relay. It declares reusable units in GraphQL for components for shared use
amongst components, while retaining the ability to be combined into one single
fetch.
...
<Alert className="mt-5" title="Reuse query components">
  ...
  selections that the query component needs. This makes sure we are making the
  exact same selections, keeping the normalized cache fresh and consistent.
</Alert>
...
This chart shows the flow of data when performing an optimistic updates.
...
In Suspense mode, mutations selections should, instead, be made inside a
callback function.
...
Try to make selections with a `prepare` function shared with query components,
read more about Suspense on queries [here](/react/read#suspense).
