...
<Alert title="Under the hood">
  You are actually rendering off of the cached data. GQty captures what is being
  accessed while rendering, combines what is being accessed into a single
  GraphQL query, fetches them, and then triggers a re-render for you.
</Alert>
...
<Alert className="mt-5" title="Keep arrays immutable">
  When you start learning React, it may be tempting to mutate the arrays during
  render, such as sorting or filtering. You should avoid them because it is easy
  to create pitfalls hard to notice. Instead, let the API do it from the
  backend.

See how to pass GraphQL inputs in GQty in the next section.
...
Not to be confused to altering on render, re-formatting is instead a handy way
...
placeholders that matches the schema data structure. It comes in handy when you
want to display skeleton/glimmer loaders that matches your UI with no extra
efforts.
...
export default function Profile() {
  return (
    <>
      <h1>Hello, {user.name ?? nameSkeleton}!</h1>
      <ol>
...
On the surface it looks like your `<Suspense />` already caught the fetch, so it
is tempting to stop here and call it a day. But enabling `suspense` without
...
such that it trigger suspense before the first render takes place.

```tsx {5-10} filename="src/Profile.tsx"
import { useQuery } from "./gqty";
...
export function Profile() {
  const { me } = useQuery({
    prepare({ query: { me } }) {
      me.name;
      me.friends({ limit: 10, offset: 20 }).map((user) => {
        user.name;
      });
    },
    suspense: true,
...
With Suspense enabled, errors during data fetch will be thrown in the next
...
where you think more in terms of the component tree as in `<Suspense>` more than
...
in the new core because they are useful in some cases.
...
If you have thoughts about these legacy hooks, please don't hesitate to chime in
our Discord server.
