...
being accessed, combine selections with the same context, fetching them with
...
checks the cache freshness of accessed data, it fetches only when cache data
needs it AND the current cache policy allows it.

Since it is only a validity check, this allows more frequent refetching
strategy. Window focus refetching, reconnect refetching and interval refetching
are a good example of this behavior.
...
<Image src={svgNormalization1} alt="fig1: Cache Normalization Step 1" />

They will be merged into the same object reference, and kept in a global store.

<Image src={svgNormalization2} alt="fig2: Cache Normalization Step 2" />

This global store will be serialised along with the main data cache during
...
<Image src={svgNormalization3} alt="fig3: Cache Normalization Step 3" />
...
<Image src={svgNormalization4} alt="fig4: Cache Normalization Step 4" />
