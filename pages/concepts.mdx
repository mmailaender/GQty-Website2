...
checks the cache freshness of accessed data, it fetches only on cache miss
AND the current cache policy allows it.
...
simply use `query.$refetch(true)` in React to bypass a fresh cache and forces
a network request.
