...
If you think your use case is possible without a configuration file, tell us in
Discord or GitHub!

## Where?

We use `cosmiconfig`, so we search for configuration files in the following
order:
...
1. In the directory `.config/`, the file `gqtyrc`, `gqtyrc.json`, `gqtyrc.yaml`,
   `gqtyrc.yml`, `gqtyrc.js` or `gqtyrc.cjs`, or
